...
2. **First publish** (crate must exist before Trusted Publishing can be attached):
   ...
   - Set GitHub Actions secret `CARGO_REGISTRY_TOKEN` and run the **Publish** workflow (`crates` job), or
...
The `crates` job in [`.github/workflows/publish.yml`](../../.github/workflows/publish.yml)
(via [`.github/actions/publish-crates`](../../.github/actions/publish-crates/action.yml)) runs on:
...
4. Confirm the **Publish** workflow `crates` job succeeded.
