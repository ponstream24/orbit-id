...
[v1 ワイヤ互換](stable-release-criteria.md) を守る）:

| 成分 | 意味 | 例 |
| --- | --- | --- |
| **X**（major） | **破壊的**変更 | 公開 API の非互換変更。既存 ID の解釈を変える encode/decode 変更（通常は新 major — Stable 基準を参照） |
...
- **Bump release PR** Action（`.github/workflows/release.yml`）は公開カットの一括 bump PR を開く。主に **X** / **Y**（意図的な lockstep の **Z**）向け。本当に言語だけの **Z** は手動 bump（または後で Action に個別トグルを足す）。
...
推奨: **Bump release PR**（`.github/workflows/release.yml`）を
