...
4. Type 値は [Type field guidance](type-registry.md) に従う。プロダクト固有の Type マップはこの
   リポジトリに置かない。
...
stable `v1.0.0` 公開済みです。v1 major 内では既存の 64-bit 値の解釈を変更しません。追加の文書、
Type（未割当値のみ）、ライブラリ機能は minor / patch で出せます。
