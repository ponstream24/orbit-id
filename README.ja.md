...
> [!IMPORTANT]
> 現在は仕様策定段階です。実装とパッケージ API はまだ提供していません。
...
初回マイルストーンは specification-first です。言語非依存の形式と共通テストベクトルを先に固め、
その後に参照実装と多言語パッケージへ進みます。Redis を使う場合も Node lease の管理に限定し、
ID 生成は各 Orbit ノード内で完結させます。予定のモノレポ構成と API 表面は
