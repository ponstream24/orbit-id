...
> [!IMPORTANT]
> Orbit ID v1 は stable（`v1.0.0`）です。実装は [`@orbit-id/core`](packages/core) と
> [`@orbit-id/typescript`](packages/typescript) を参照してください。
...
- [Library API (Draft)](docs/ja/library-api.md)
...
Orbit ID v1 は stable です。まずは [`@orbit-id/core`](packages/core) または
[`@orbit-id/typescript`](packages/typescript) を参照してください。Redis を使う場合も Node lease の
管理に限定し、ID 生成は各 Orbit ノード内で完結させます。残りのパッケージは
[Roadmap](docs/ja/roadmap.md) を参照してください。
